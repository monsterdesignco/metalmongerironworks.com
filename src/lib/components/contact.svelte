        ...
        <div class="space-y-8 text-center md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-12 md:space-y-0">
            {#each cards as card}
                <div>
                    ...
                    <p class="mb-2 text-xl font-heading font-bold dark:text-white">{card.title}</p>
                    ...
                    {#if card.link}
                        <a href={card.link.link} class="font-semibold text-primary-600 dark:text-primary-500 hover:underline">{card.link.text}</a>
                    {/if}
                </div>
            {/each}
